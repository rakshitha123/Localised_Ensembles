# Pooled Regression - Ensemble of Experts
...
def get_forecasts(final_rank_mat, NUM_OF_EXPERTS, lag):
    ...
    for model in range(NUM_OF_EXPERTS):
        if(len(listOfAssignmentIndices[model])>0):
            ...
            forecasts = get_regression_forecasts(listOfAssignmentIndices[model], lag)
            print("forecasts")
...
def get_regression_forecasts(indices, current_lag):
    out = subprocess.check_output(
        ["Rscript", "--vanilla", BASE_DIR + "pooled_regression/regression.R", original_data_file, str(forecast_horizon),
         str(current_lag), str(address_near_zero_instability), str(integer_conversion), str(0), str(list(indices))], stderr=subprocess.STDOUT)
...
def train_model(indices, current_lag):
    out = subprocess.check_output(["Rscript", "--vanilla", BASE_DIR + "pooled_regression/regression.R", original_data_file, str(forecast_horizon),
         str(current_lag), str(address_near_zero_instability), str(integer_conversion), str(1), str(list(indices))], stderr=subprocess.STDOUT)
    out = out.split()
    out = out[-numOfSeries:]
...
if __name__ == '__main__':
    print("Ensemble of Experts with Pooled Regression")
    ...
    for lag in lags:
        ...
        for iter in range(MAX_NUM_OF_ITERATIONS):
            for model in range(NUM_OF_EXPERTS):
                if ((not is_early_stopping) or (is_early_stopping and (not (model in badModels)))):
                    ...
                    smape_errors =  train_model(listOfAssignmentIndices[model], lag)
                    #print(smape_errors)
        ...
        final_forecasts = get_forecasts(final_rank_mat,NUM_OF_EXPERTS, lag)

        regression_forecasts_full_file_path = BASE_DIR + model_testing_configs.REGRESSION_FORECASTS_DIRECTORY + model_identifier
        validation_error_file_path = BASE_DIR + model_training_configs.VALIDATION_ERRORS_DIRECTORY + model_identifier

        with open(regression_forecasts_full_file_path, "w") as output:
        ...
        error_file_name = model_identifier
        regression_forecasts_file_path = model_testing_configs.REGRESSION_FORECASTS_DIRECTORY + model_identifier

        subprocess.call(
            ["Rscript", "--vanilla", BASE_DIR + "error_calculator/other_model_evaluation.R", dataset_name, regression_forecasts_file_path, actual_results_file, original_data_file,
             "regression_ensemble_of_experts", str(address_near_zero_instability), str(seasonality_period), str(lag)],
