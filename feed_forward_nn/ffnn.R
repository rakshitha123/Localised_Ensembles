#Use with FFNN ensemble of experts code#
...
indices <- args[7]
num_of_hidden_nodes <- as.numeric(args[8])
decay <- as.numeric(args[9])
...
fit_model <- function(fitting_data) {
  ...
  # fit the model
  model <- nnet(formula = formula, data = fitting_data, size = num_of_hidden_nodes, decay = decay, linout = TRUE, trace = FALSE)
