...
calculate_errors <- function(forecast_file, actual_forecasts_file, original_data_file, technique, address_near_zero_insability, seasonality_period){
  actual_forecasts_file = paste(base_dir, actual_forecasts_file, sep="")

  SMAPE_file_full_name_all_errors <- paste0(errors_dir, "all_smape_errors_", technique, ".txt")
  MASE_file_full_name_all_errors <- paste0(errors_dir, "all_mase_errors_", technique, ".txt")
